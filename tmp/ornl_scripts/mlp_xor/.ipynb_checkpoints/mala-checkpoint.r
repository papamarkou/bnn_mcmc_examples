...
accepted <- read.table(file.path(OUTDIR, 'mala_accepted.txt'), header=FALSE)
...
for (i in seq(1, npars)) {
  print(i)
  chain <- read.table(file.path(OUTDIR, paste('mala_chain', str_pad(i, 2, pad='0'), '.txt', sep='')), header=FALSE)
