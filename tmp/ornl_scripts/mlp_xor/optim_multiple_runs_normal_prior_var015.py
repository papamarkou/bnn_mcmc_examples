...
OUTDIR = os.path.join("output", "optim", "multiple_runs", "normal_prior", "var015", "iters010000")
...
model.prior = Normal(
    ...
    np.sqrt(15)*torch.ones(model.num_params(), dtype=model.dtype)
)
...
def accept_optimization(logit, lb, ub):
    if (torch.any(torch.isnan(logit)) or (not (
        ((logit[0] < lb) and (logit[1] > ub) and (logit[2] > ub) and (logit[3] < lb)) or
        ((logit[0] > ub) and (logit[1] < lb) and (logit[2] < lb) and (logit[3] > ub))
    ))):
        return False
    else:
        return True
...
while (i < num_optimizations):
    ...
    if accept_optimization(logit, 0.1, 0.9):
