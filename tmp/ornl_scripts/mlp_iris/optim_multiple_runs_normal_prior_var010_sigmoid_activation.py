...
OUTDIR = os.path.join("output", "optim", "multiple_runs", "normal_prior", "var010", "sigmoid_activation", "iters001000")
...
model = mlp.MLP(
    hparams=hparams, loss=lambda x, y: nn.CrossEntropyLoss(reduction='sum')(x, torch.argmax(y, 1)), dtype=torch.float)
model.prior = Normal(
