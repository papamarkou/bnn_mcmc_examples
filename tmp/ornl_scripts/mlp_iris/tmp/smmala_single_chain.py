...
plt.plot(sampler.chain.get_theta(1))
...
# Save tensor in file
with open(os.path.join("output", "smmala", "single_chain", "chain.csv"), 'w') as file:
    np.savetxt(file, chain.cpu().detach().numpy(), delimiter=',', newline='\n', header='')

## Save acceptance diagnostic for simulated Markov chain

with open(os.path.join("output", "smmala", "single_chain", "accepted.txt"), 'w') as file:
...
with open(os.path.join("output", "smmala", "single_chain", "runtime.txt"), 'w') as file:
